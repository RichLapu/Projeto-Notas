            ...
            <div class="card p-5 bg-body-tertiary border border-secondary df-fullscreen-card">
                ...
                <div class="row mb-4 align-items-center hide-in-df">
                    <div class="col d-flex align-items-center">
                        ...
                        <span id="autosave-status" class="badge bg-secondary d-none ms-4 mt-2">Salvo automaticamente</span>
                        ...
                        <div class="col">
                            ...
                            <div class="hide-in-df">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="text_title" value="{{ old('text_title', $note->title) }}">
                                    @error('text_title')
                                    ...
                                    <div class="d-flex gap-3 flex-wrap">
                                        @foreach($categories as $category)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="categories[]" value="{{ $category->id }}" id="cat_{{ $category->id }}" {{ $note->categories->contains($category->id) ? 'checked' : '' }}>
                                ...
                                <div class="form-check form-switch mb-4">
                                    <input class="form-check-input" type="checkbox" role="switch" id="is_protected" name="is_protected" value="1" {{ $note->is_protected ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <input type="hidden" name="text_note" id="text_note" value="{{ old('text_note', $note->text) }}">
                                
                                <div id="editor" class="bg-body text-body" style="height: 300px;">
                                    {!! old('text_note', $note->text) !!}
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 4. Lógica do Autosave
        const noteId = document.getElementById('note_id');
        const titleInput = document.querySelector('input[name="text_title"]');
        const statusBadge = document.getElementById('autosave-status');
        
        setInterval(() => {
            if (!titleInput.value || !noteId) return;

            const content = quill.root.innerHTML; 
            const formData = new FormData();
            
            formData.append('_token', '{{ csrf_token() }}');
            formData.append('id', noteId.value);
            formData.append('title', titleInput.value);
            formData.append('text_note', content); 

            fetch('{{ route("autosave") }}', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
                    statusBadge.innerText = 'Salvo às ' + data.time;
            ...
            .catch(error => console.error('Erro no autosave:', error));
